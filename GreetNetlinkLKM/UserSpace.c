...
static void* _start_kernel_data_receiver_thread (void* arg)
{
  ...
  recv = (struct nlmsghdr*)calloc (1, NLMSG_HDRLEN + NLMSG_SPACE (MAX_PAYLOAD));

  do {
    memset (recv, 0, NLMSG_HDRLEN + NLMSG_SPACE (MAX_PAYLOAD));

    iov.iov_base = (void*)recv;
    ...
    recv          = outermsghdr.msg_iov->iov_base;
    char* payload = NLMSG_DATA (recv);
...
void start_kernel_data_receiver_thread (thread_arg_t* thread_arg)
{
  ...
  pthread_create (&recv_pkt_thread, &attr, _start_kernel_data_receiver_thread,
                  (void*)thread_arg);
}
...
int main (int argc, char** argv)
{
  ...
  while (1) {
    ...
    case 1: {
      ...
      greet_kernel (sock_fd, user_msg, sizeof (user_msg));
    } break;
    case 2: exit_userspace (sock_fd); break;
    default:;
    }
  }
  return 0;
